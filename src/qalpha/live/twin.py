...
#: materially different one needs the null re-run. **Also data-dependent**: re-running after a price
#: refresh moved it 8,362,315 → 8,411,106, about 0.6%. That drift is visible only because every run
#: is recorded (``backtest/runstore.py``); the same parameters on the same data reproduce exactly.
#: See reports/PHASE4_BACKTEST.md.
BACKTEST_NOISE_FLOOR = Decimal("8411106")
