...
def test_hedging_half_a_book_needs_twice_a_lot_s_notional() -> None:
    ...
    One contract at index 25,000 is 75 x 25,000 = Rs 18.75L of exposure. Offsetting HALF a book with
    it therefore needs a Rs 37.5L book, not a Rs 18.75L one.
    """
    a = hedge_availability(0, 25_000, lot_size=75, hedge_ratio=0.5)
    assert a.lot_notional == 75 * 25_000
    assert a.book_value_needed == a.lot_notional / 0.5
    assert a.book_value_needed == 3_750_000
...
def test_lots_are_whole_because_halves_cannot_be_bought() -> None:
    a = hedge_availability(3_750_000 * 2.9, 25_000)
...
def test_the_contract_size_is_flagged_as_needing_verification() -> None:
    ...
    assert "Verify against current NSE contract specs" in src
